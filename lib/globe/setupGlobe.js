...
export default function setupGlobe(locationInfoArray) {
  /**
   *
   * @param {url} param
   */
  function FsGlobe() {
    ...
    // );
    const team = fetchDataFromCollection(locationInfoArray)
  ...
  function fetchDataFromCollection(collectionWrapper) {
    ...
    // accepting data from react- modified - by sagar
    return collectionWrapper.map((item) => {
      const { latitude, longitude } = item.geoPin
      return {
